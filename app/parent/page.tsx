...
type ParentMaterial = { href: string; title: string; description: string; language?: string };
...
const PART_2_APP: LearningApp = {
  ...
  subject: 'NMG',
  grade_label: '6. Klasse',
  topic_labels: ['Überblick & Vergleich', 'Buddhismus', 'Buddhas Leben & Lehre', 'Hinduismus', 'Hinduistische Gottheiten'],
};
...
const MATERIALS: Record<ParentLanguage, ParentMaterial[]> = {
  de: [
...
export default function ParentPage() {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    const catalogue = (appsResult.data ?? []) as LearningApp[];
    const apps = catalogue.some((app) => app.app_key === DEFAULT_APP_KEY)
      ? catalogue
      : [...catalogue, PART_2_APP];
    apps.sort((a, b) => a.app_key === DEFAULT_APP_KEY ? -1 : b.app_key === DEFAULT_APP_KEY ? 1 : a.title.localeCompare(b.title));
  ...
  const selectedTitle = selectedAppKey === PART_1_APP_KEY ? c.appTitle : selectedAppKey === DEFAULT_APP_KEY ? c.appTitle2 : selectedApp?.title ?? c.learningApp;
  const selectedMaterials = selectedAppKey === PART_1_APP_KEY
    ? MATERIALS[language]
    : selectedAppKey === DEFAULT_APP_KEY
      ? PART_2_MATERIALS[language]
      : [];
  ...
  return <ParentShell>
    ...
    <section className="appSwitcher" aria-label={c.chooseApp}>{learningApps.map((app) => <button key={app.app_key} className={app.app_key === selectedAppKey ? 'active' : ''} onClick={() => { setSelectedAppKey(app.app_key); setPairingCode(''); }}><span>{app.subject}</span><strong>{app.app_key === PART_1_APP_KEY ? c.appTitle : app.app_key === DEFAULT_APP_KEY ? c.appTitle2 : app.title}</strong><small>{app.app_key === PART_1_APP_KEY || app.app_key === DEFAULT_APP_KEY ? c.grade : app.grade_label}</small></button>)}</section>
    <div className="selectedAppTitle"><span>{selectedApp?.subject ?? c.subject}</span><h2>{selectedTitle}</h2></div>
    {!!selectedMaterials.length && <section className="materialsPanel"><div className="materialsHeading"><div><p className="eyebrow">{c.materials}</p><h2>{c.materialsTitle}</h2></div><p>{c.materialsCopy}</p></div><div className="materialsGrid">{selectedMaterials.map((item) => <a className="materialCard" href={item.href} download key={item.href}><span className="materialLanguage">{item.language ?? language.toUpperCase()}</span><strong>{item.title}</strong><small>{item.description}</small><b>{c.download}</b></a>)}</div></section>}
